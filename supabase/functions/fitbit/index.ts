...
async function syncActive(accessToken: string, d: string) {
  // Filter-datatype moet in snake_case (Google Health API), niet camelCase.
  const filter = `active_energy_burned.interval.civil_start_time >= "${d}T00:00:00" AND active_energy_burned.interval.civil_start_time < "${shiftDay(d, 1)}T00:00:00"`;
  ...
  let kcal = 0;
  for (const dp of dps) {
    const e = (dp.activeEnergyBurned || dp.active_energy_burned || dp["active-energy-burned"] || dp) as Record<string, unknown>;
