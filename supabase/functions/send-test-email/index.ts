// ============================================
// BRIGHTLY - Edge Function: send-test-email
// Stuurt een testmail via emailit (https://api.emailit.com/v2/emails) naar het
// e-mailadres van de INGELOGDE gebruiker. Geen open verzend-endpoint (geen vrije 'to').
// Secrets: EMAILIT_API_KEY (verplicht), EMAILIT_FROM, EMAILIT_REPLY_TO (optioneel).
// ============================================
...
/** Centrale verzendhelper — vanilla REST naar emailit. text is verplicht (anti-spam). */
...
function testHtml(): string {
  const GREEN = "#2FA45F", DARK = "#1E5C3A";
  return `<!DOCTYPE html>
<html lang="nl">
<head>
  <meta charset="UTF-8">
  ...
  <title>Testmail Brightly</title>
</head>
<body style="margin:0;padding:0;background:#f5f5f5;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,Arial,sans-serif;">
  <div style="display:none;max-height:0;overflow:hidden;font-size:1px;line-height:1px;color:#f5f5f5;opacity:0;">Je e-mailkoppeling met emailit werkt.</div>
  <table width="100%" cellpadding="0" cellspacing="0" role="presentation" style="background:#f5f5f5;">
    <tr><td align="center" style="padding:24px 12px;">
      <table width="600" cellpadding="0" cellspacing="0" role="presentation" style="max-width:600px;width:100%;background:#ffffff;border-radius:10px;overflow:hidden;">
        <tr><td bgcolor="${DARK}" style="background:${DARK};padding:28px 32px;text-align:center;">
